...
//GENERATE OPTIONS FOR SELECTION/LIST BOX FROM AN0. ARRAY
...
if ($varCountry!="" && $varDisplay == 'state') {
	...
	if (($varCountry == "98")||($varCountry == "222") ||($varCountry == "195") || ($varCountry == "162")) {
		...
		if($varCountry == "98") {
			...
			$varStateList .= "<select class=\"srchselect\" name=\"residingState\" size=\"1\" tabindex=\"".$varTabIndex++."\" onChange=\"ajaxCityCall('".$confValues['SERVERURL']."/register/country/regstatecity.php','".$varTabIndex."');residingstateChk();\" onblur=\"residingstateChk();\"><option value=\"0\">--- Select ---</option>";
...
if ($varDisplay == 'city') {
	...
	if ($varCountry == "98") { 
		...
		$stateList = $$residingPakiCityStateMappingList[$varState]; 
		asort($stateList);
		$varTabIndex++;
		$varCityList	.= '<select name="residingCity" size="1" tabindex="'.$varTabIndex++.'" class="srchselect"  onBlur="residingcityChk();"><option value="0">--- Select ---</option>';
		foreach($stateList as $varIndex => $varValue) {
...
if ($varDisplay == 'citypakistan') {	
	 $tilted=explode('~',$varState);
     $stateList=array();		
     foreach($tilted as $value){
		$stateList = $stateList + $$residingPakiCityStateMappingList[$value];
	 }
		
echo '<div class="srchdivlt smalltxt fleft tlright">Residing Pakistan City</div><div class="srchdivrt smalltxt fleft"><div class="fleft"><select class="srchselect" id="residingPakCityTemp" name="residingPakCityTemp[]" ondblclick="moveOptions(this.form.residingPakCityTemp, this.form.pakistanCity)" size="4" multiple tabindex="'.$varTabIndex++.'">';
getValuesFromArray($stateList, "Select residing Pakistan City", "0", "");
echo '</select></div><div class="fleft padlr10 padt5"><input type="button" value=" > " class="button" onclick="moveOptions(this.form.residingPakCityTemp, this.form.pakistanCity)"><br><img src="'.$confValues['IMGSURL'].'/trans.gif" width="1" height="5" /><br><input type="button" value=" < " class="button" onclick="moveOptions(this.form.pakistanCity, this.form.residingPakCityTemp)"></div><div class="fleft"><select class="srchselect" size="4" multiple id="pakistanCity" name="pakistanCity[]" tabindex="11" ondblclick="moveOptions(this.form.pakistanCity, this.form.residingPakCityTemp)"></select></div><br clear="all"><font class=\'opttxt\'>Hold CTRL key to select multiple items.</font></div>';
}
...
//pakistan state & city edit 
if ($varDisplay == 'cityeditpakistan') {	
	 $tilted=explode('~',$varState);
     $stateList1=array();	
     foreach($tilted as $value){
		 if($value != 0)
	     $stateList1 = $stateList1 + $$residingPakiCityStateMappingList[$value];
	 }		

echo '<div class="srchdivlt smalltxt fleft tlright">Residing Pakistan City</div><div class="srchdivrt smalltxt fleft"><div class="fleft"><select class="srchselect" id="residingPakCityTemp" name="residingPakCityTemp[]" ondblclick="moveOptions(document.getElementById(\'residingPakCityTemp\'), document.getElementById(\'pakistanCity\'));fnAnyChk(document.frmProfile.residingPakCityTemp,document.frmProfile.pakistanCity);" size="4" multiple tabindex="'.$varTabIndex++.'">';
getValuesFromArray($stateList1, "Any", "0", "");
echo '</select></div><div class="fleft padlr10 padt5"><input type="button" value=" > " class="button" onclick="moveOptions(document.getElementById(\'residingPakCityTemp\'), document.getElementById(\'pakistanCity\'));fnAnyChk(document.frmProfile.residingPakCityTemp,document.frmProfile.pakistanCity);"><br><img src="'.$confValues['IMGSURL'].'/trans.gif" width="1" height="5" /><br><input type="button" value=" < " class="button" onclick="moveOptions(document.getElementById(\'pakistanCity\'), document.getElementById(\'residingPakCityTemp\'));"></div><div class="fleft"><select class="srchselect" size="4" multiple id="pakistanCity" name="pakistanCity[]" tabindex="11" ondblclick="moveOptions(this, document.getElementById(\'residingPakCityTemp\'));">';



if($stateList1==0){echo $objSearch->getOpionalValues(0, array(0=>'Any'));} else {echo $objSearch->getOpionalValues($varSelectedCity, $stateList1);}
echo'</select></div><br clear="all"><font class=\'opttxt fleft\'>Hold CTRL key to select multiple items.</font></div>';
} 
