...
var arrReligiosValueforReligion=new Array(2,10,11,20,21,22,23);
...
if (month>0)
{
	//if ((change == 'month' && month == 2) || (change == 'year' && month == 2))
	if ((change == 'month' && month) || (change == 'year' && month))
	{
		var i = 31;
		var flag = true;
		while(flag)
...
if (parseInt(document.frmRegister.religionfeature.value)==1 && parseInt(document.frmRegister.religionOption.value)>1 ) {
	...
	else if(religionId == 23 && document.frmRegister.religionOthers.value == '') {
	  document.getElementById('religionspan').innerHTML="Enter the religion of the prospect";
	  document.frmRegister.religionOthers.focus();return false;
	}
...
if(parseInt(frmRegister.communityId.value)==2007){
     var frmRegister = document.frmRegister;
	var countryId	= parseInt(frmRegister.placeofbirth.options[frmRegister.placeofbirth.selectedIndex].value);
...
if (countryId==98 || countryId==222 || countryId==195 || countryId==162) {
	if (frmRegister.residingState.selectedIndex==0) {
		if(countryId==195){
		document.getElementById('residingstatespan').innerHTML="Select the district of the prospect";
		}else{
...
if(parseInt(frmRegister.motherTongue.options[frmRegister.motherTongue.selectedIndex].value) == 9997)
{
 if(frmRegister.Mother_TongueText.value=="")
 {document.getElementById('mothertonguespan').innerHTML="Enter the mother tongue of the prospect";
 frmRegister.Mother_TongueText.focus();return false;}
 else{document.getElementById('mothertonguespan').innerHTML="";}
}	
	...
	if (parseInt(document.frmRegister.casteOption.value)==1) {} else {
	  ...
	  if (parseInt(document.frmRegister.casteOption.value)>1) {
		  ...
		  if(casteId == 9997) {
			  if (document.frmRegister.casteOthers.value=="") {
					//document.getElementById('castespan').innerHTML="Enter the "+document.frmRegister.castelabel.value+" of the prospect";
					document.getElementById('castespan').innerHTML="Enter the "+document.getElementById('branchDiv').innerHTML+" of the prospect";
					return;
			  }
			  else { document.getElementById('castespan').innerHTML=''; }
		  }
...
function countryChk() {
	...
	if(countryId==195){funSrilankanReside(countryId);} 
		...
		document.getElementById('residspan').innerHTML="Residing state<font style=\"color:#ff0000\">*</font>";
		document.getElementById('residcitydist').innerHTML="Residing City / District<font style=\"color:#ff0000\">*</font>";
		}
...
function funSrilankanReside(countryId) {  // Residing state Lable change for Srilankan 
  if(countryId==195){ 
	  document.getElementById('residspan').innerHTML="District<font style=\"color:#ff0000\">*</font>";
	  document.getElementById('residcitydist').innerHTML="Residing City<font style=\"color:#ff0000\">*</font>";
    }
	else{
	  document.getElementById('residspan').innerHTML="Residing state<font style=\"color:#ff0000\">*</font>";
	  document.getElementById('residcitydist').innerHTML="Residing City / District<font style=\"color:#ff0000\">*</font>";
...
function residingstateChk() {
	...
	if (cId==98 || cId==222 || cId==195 || cId==162) {
		if (frmRegister.residingState.selectedIndex==0) {
			if(cId==195){
			document.getElementById('residingstatespan').innerHTML="Select the district of the prospect";return;
			}else{
...
function ajaxStateCall(url,tabIndex) {
	...
	if (country!=98 || country!=162 ) { ajaxCityCall(url,tabIndex); }
...
//For Pakistan State City Map
var objPakistanCityCall;
function ajaxPakistanCityCall(url,tabIndex) {
	objPakistanCityCall = AjaxCall();	
	var statval=new Array();
	var residingPakistan=document.frmRegister.residingPakistan;	
	var residingcount=residingPakistan.options.length;	
	if(residingcount>0){
	for(var i=0; i<residingPakistan.options.length; i++){			
	statval[i]=document.frmRegister.residingPakistan.options[i].value;
	}
	statval=statval.join("~");
	alert(statval);
	var parameters	=  "tabIndex="+tabIndex+"&state="+statval+"&display=citypakistan&rand="+Math.random();
	 objPakistanCityCall.onreadystatechange = regCityPakistanList;
	 objPakistanCityCall.open('POST', url, true);
	 objPakistanCityCall.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	 objPakistanCityCall.setRequestHeader("Content-length", parameters.length);
	 objPakistanCityCall.setRequestHeader("Connection", "close");
	 objPakistanCityCall.send(parameters);
	return objPakistanCityCall;
	}
}

function regCityPakistanList() {
	if (objPakistanCityCall.readyState == 4 && objPakistanCityCall.status == 200) {
		document.getElementById('cityPakistanList').innerHTML	= objPakistanCityCall.responseText;
	}
}
...
function motherTongueOther(selVal){ 
	if(selVal=="9997"){
			...
			document.frmRegister.Mother_TongueText.focus();
	} else { 
		document.frmRegister.Mother_TongueText.value="";
...
function funReligion(submittedCasteId){
	...
	// Condition for showing religious value option 
	if(!checkValueInArray(arrReligiosValueforReligion,religionId)){
        document.getElementById('religiousValuesDiv').style.display = "none";  		
	}else{
		document.getElementById('religiousValuesDiv').style.display = "inline"; 	
	}

   if(casteFeature == 1) {
   document.getElementById('castespan').innerHTML='';
   }
   document.frmRegister.religionOthers.value ='';
   if(religionId == 23) {
	...
	//12//if ((religionId=='8') || (religionId=='9')) { funSubCasteText(); }
...
function religionOthersChk() {
 var religionId	= document.frmRegister.religion.options[document.frmRegister.religion.selectedIndex].value;
 var religionText= document.frmRegister.religionOthers.value;
