...
$varActFields	= array("Email");
$varActCondtn	= " where RMUserid=".$rmclass->slave->doEscapeString($_COOKIE['rmusername'],$rmclass->slave);
...
$rmmailid = $loginrow[0]['Email'];
 ...
 if(isset($_REQUEST['do_submit'])){ 
	...
	$varInsertVal	= array($rmclass->master->doEscapeString($_COOKIE['rmusername'],$rmclass->master),$rmclass->master->doEscapeString($_POST['usrmatriid'],$rmclass->master),$rmclass->master->doEscapeString($_POST['toid'],$rmclass->master),$rmclass->master->doEscapeString($subject,$rmclass->master),$rmclass->master->doEscapeString($message,$rmclass->master),$rmclass->master->doEscapeString($attachfilepath,$rmclass->master),'NOW()');
