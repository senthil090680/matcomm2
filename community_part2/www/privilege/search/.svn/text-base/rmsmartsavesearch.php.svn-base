...
function parse_cookie() { // returns the savesearch cookie value as array
	...
	//list($COOKIESAVESEARCH['savesearch1'], $COOKIESAVESEARCH['savesearch2'], $COOKIESAVESEARCH['savesearch3'],$COOKIESAVESEARCH['savesearch4']) = split("~", $COOKIEINFO['SAVESEARCH']['SAVESEARCH'], 4);
	$ck_info=split("~", $COOKIEINFO['SAVESEARCH']['SAVESEARCH']);
	$save_ck=0;
	for($ck_inc=0;$ck_inc<count($ck_info);$ck_inc++){
		 $save_ck=$ck_inc+1;
		 $COOKIESAVESEARCH['savesearch'.$save_ck]=$ck_info[$ck_inc];
	}
...
function getSaveSearchNames($rmid='') { // get the savesearch names display in right side
	global $COMMONVARS,$debug_it, $COOKIESAVESEARCHINFO, $COOKIEINFO,$GETDOMAININFO;
	...
	if($COOKIEINFO['LOGININFO']['MEMBERID'] != "") {	
		...
		if(count($COOKIESAVESEARCHINFO)>0) {
			foreach($COOKIESAVESEARCHINFO as $SAVENUM=>$COOKIESEARCHARRAY) {
				...
				//echo $COOKIESEARCHARRAY['searchid'];
				//echo $COOKIESEARCHARRAY['searchname'];
				...
				$dbsavesearch="select Comments from ".$DBNAME['MATRIMONY'].".".$DOMAINTABLE[strtoupper($GETDOMAININFO['domainnameshort'])]['RMSAVESEARCH']." where MatriId='".$COOKIEINFO['LOGININFO']['MEMBERID']."' and SearchId=".$COOKIESEARCHARRAY['searchid']." and SearchName='".$COOKIESEARCHARRAY['searchname']."'";
				$savesrch_slave_con->select($dbsavesearch);
				$savesrch_comment=$savesrch_slave_con->fetcharray();
				$comments=$savesrch_comment['Comments'];

			   	$save_search_content1='<a href="#"  class=normaltext3 onmouseout="hidetip();" onmouseover="showhint(\''.$comments.'\',this,event,\'130\');">'.stripslashes($COOKIESEARCHARRAY['searchname']).'</a>';

				 $save_search_content .= "<div><div id='savedsearch_".$COOKIESEARCHARRAY['searchid']."'><div class='smalltxt'>".$save_search_content1."</div>";
...
function getSavedSearchHiddenquery() { // returns the query to get hidden forms of saved search
	...
	$debug_it['err'] .= $debug_it['br'] .$QUERY = "select MatriId, SearchId, SearchType, SearchName, Language, Gender, MaritalStatus, HaveChildren, StAge, EndAge, StHeight, EndHeight, Religion, Caste, SubCaste, Manglik, Star, Raasi, EatingHabits, MotherTongue, PhysicalStatus, Education, OccupationCategory,OccupationSelected ,Citizenship, Country, ResidingIndia, ResidingDistrict,ResidingUSA, ResidentStatus, Keywords, Days, PostedAfter, DateOpt, PhotoOpt, HoroscopeOpt, IgnoreOpt, ContactOpt, DisplayFormat, PerPage, BodyType, Complexion, IncomeCurrency ,AnnualIncome, Drinking, Smoking, Divstatus from ".$DBNAME['MATRIMONY'].".".$DOMAINTABLE[strtoupper($GETDOMAININFO['domainnameshort'])]['RMSAVESEARCH']." where MatriId = '".$COOKIEINFO['LOGININFO']['MEMBERID']."'";
...
function updatesavesearch() { // perform insert or delete action of savesearch
	...
	$table_savename = $DBNAME['MATRIMONY'].".".$DOMAINTABLE[$domainname]['RMSAVESEARCH'];
...
function queryformation($action,$SearchId,$search_escape_name) {// return the posted save search values and query values
	...
	if($action == "update") {
		...
		if($save_type == 'R') {
			...
			$dbfields = "MatriId,SearchId,SearchType,SearchName,Language,Gender,MaritalStatus,StAge,EndAge,StHeight,EndHeight,Religion,Caste,SubCaste,Education,Citizenship,Country,Days,PostedAfter,DateOpt,PhotoOpt,HoroscopeOpt,IgnoreOpt,ContactOpt,DisplayFormat,PerPage,Comments";
			
			$savesearchvalues = "'".$COOKIEINFO['LOGININFO']['MEMBERID']."',".$SearchId.",'".$save_type."','".$search_escape_name."','".$language."','".$gender."','".$marital."',".$stage.",".$endage.",'".$stheight."','".$endheight."','".$religion."','".$caste."','".$subcaste."','".$education."','".$citizen."','".$country."','".$days."','".$postdate."','".$dateopt."','".$photoopt."','".$horoscope."','".$ignoropt."','".$contactopt."','".$display_format."',".$perpage.",'".$savecomments."'";


		} else {
			$dbfields = "MatriId,SearchId,SearchType,SearchName,Language,Gender,MaritalStatus,HaveChildren,StAge,EndAge,StHeight,EndHeight,Religion,Caste,SubCaste,Manglik,EatingHabits,Drinking,Smoking,MotherTongue,PhysicalStatus,Education,OccupationCategory,OccupationSelected,Citizenship,Country,ResidingIndia,ResidingUSA,ResidingDistrict,Star,ResidentStatus,Keywords,Days,PostedAfter,DateOpt,PhotoOpt,HoroscopeOpt,IgnoreOpt,ContactOpt,DisplayFormat,PerPage,Divstatus,Comments";

			$savesearchvalues = "'".$COOKIEINFO['LOGININFO']['MEMBERID']."',".$SearchId.",'".$save_type."','".$search_escape_name."','".$language."','".$gender."','".$marital."',".$havechildren.",".$stage.",".$endage.",'".$stheight."','".$endheight."','".$religion."','".$caste."','".$subcaste."','".$manglik."','".$eating."','".$drinking."','".$smoking."','".$mothertongue."','".$physical_status."','".$education."','".$occupation_cat."','".$occupation."','".$citizen."','".$country."','".$res_india."','".$res_usa."','".$res_city."','".$star."','".$res_status."','".$keyword."','".$days."','".$postdate."','".$dateopt."','".$photoopt."','".$horoscope."','".$ignoropt."','".$contactopt."','".$display_format."',".$perpage.",'".$_status."'".",'".$savecomments."'";
