...
/*Get IP Location */
function getIptoLocation(){
$ip = IP2Location_open("/usr/local/ip2location/IP-COUNTRY-REGION-CITY.BIN", IP2LOCATION_STANDARD);
$record = IP2Location_get_all($ip, $_SERVER['REMOTE_ADDR']);
/*if($location=='R'){
$iptolocationdisplay=$record->region;
}else if($location=='C'){
$iptolocationdisplay=$record->city;
}else{
$iptolocationdisplay=$record->country_short;
}*/
$iptolocationdisplay=$record->country_short;
IP2Location_close($ip);
if($iptolocationdisplay=="-" || trim($iptolocationdisplay)==""){
	$iptolocationdisplay="US";
}
return $iptolocationdisplay;
}
/**/

/*Get IP Location like country_short,region,city */
function getIptoCRC(){ // Its Return country_short,region,city like IN^TAMIL NADU^CHENNAI
	$ip = IP2Location_open("/usr/local/ip2location/IP-COUNTRY-REGION-CITY.BIN", IP2LOCATION_STANDARD);
	$record = IP2Location_get_all($ip, $_SERVER['REMOTE_ADDR']);	
	$iptolocationregion=$record->region;
	$iptolocationcity=$record->city;
	$iptolocationcountry=$record->country_short;
	IP2Location_close($ip);
	return $iptolocationcountry."^".$iptolocationregion."^".$iptolocationcity;
}
?>