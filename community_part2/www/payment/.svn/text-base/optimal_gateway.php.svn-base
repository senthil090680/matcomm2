...
if ($varCategory !='100' && $varCategory !='110' && $varCategory !='111' && $varCategory !='112' && $varPaidStatus == '1' && $varCategory !='120' && $varNoOfDays <= 10){
	...
	$varMessage	= 'You have recently made payment for Matrimony Id <b>'.$sessMatriId.'</b>. <br>The last payment was made on <b>'.$varDatePaid.'</b>. <br>A minimum of 10 days is required for subsequent payments. Please renew your membership after this period.';
		...
		$argFieldsValue	= array($objMasterDB->doEscapeString($varOrderId,$objMasterDB),$objMasterDB->doEscapeString($sessMatriId,$objMasterDB),$objMasterDB->doEscapeString($varUserName,$objMasterDB),$objMasterDB->doEscapeString($varCategory,$objMasterDB),$objMasterDB->doEscapeString($varPackageCost,$objMasterDB),$objMasterDB->doEscapeString($varActualAmount,$objMasterDB),$objMasterDB->doEscapeString($varDiscountPercentage,$objMasterDB),$objMasterDB->doEscapeString($varDiscountFlatRate,$objMasterDB),$objMasterDB->doEscapeString($varCurrency,$objMasterDB),'\'5\'','NOW()',$objMasterDB->doEscapeString($varCurrency,$objMasterDB),$objMasterDB->doEscapeString($varActualAmount,$objMasterDB),$objMasterDB->doEscapeString($varPackageCost,$objMasterDB));
		...
		$varFields		= array('OrderId','MatriId','User_Name','Product_Id','Package_Cost','Amount_Paid','Discount','DiscountFlatRate','Currency','Gateway','Date_Paid','Display_Currency','Display_Amount_Paid','Display_Package_Cost');
		$argFieldsValue	= array($objMasterDB->doEscapeString($varOrderId,$objMasterDB),$objMasterDB->doEscapeString($sessMatriId,$objMasterDB),$objMasterDB->doEscapeString($varUserName,$objMasterDB),$objMasterDB->doEscapeString($varCategory,$objMasterDB),$objMasterDB->doEscapeString($varConvertedPackageAmount,$objMasterDB),$objMasterDB->doEscapeString($varShowAmount,$objMasterDB),$objMasterDB->doEscapeString($varDiscountPercentage,$objMasterDB),$objMasterDB->doEscapeString($varDiscountFlatRate,$objMasterDB),"'USD'",'\'5\'','NOW()',$objMasterDB->doEscapeString($varCurrency,$objMasterDB),$objMasterDB->doEscapeString($varActualAmount,$objMasterDB),$objMasterDB->doEscapeString($varPackageCost,$objMasterDB));
...
<form name="frmCreditCard" method="POST" action="https://communitymatrimony.com/payment/optimal_do.php" onSubmit="return validateCreditcard();" style="margin:0px;">
			...
			<tr>
				...
				<tr>
					...
					<tr>
						...
						<div>
							<div style="width:20px;float:left;padding-top:3px;"><input type="radio" name="vpc_card" value="AM"></div><div style="width:50px !important;width:40px;float:left;"><IMG SRC="images/pay-amex.gif"></div>
                            <div style="width:20px;float:left;padding-top:3px;"><input type="radio" name="vpc_card" value="DI"></div><div style="width:50px !important;width:40px;float:left;"><IMG SRC="images/pay-dis.gif"></div>
                        </div>
                            </td>
