...
if ($varCategory !='100' && $varCategory !='110' && $varCategory !='111' && $varCategory !='112' && $varPaidStatus == '1' && $varCategory !='120' && $varNoOfDays <= 10){
	...
	$varMessage	= 'You have recently made payment for Matrimony Id <b>'.$sessMatriId.'</b>. <br>The last payment was made on <b>'.$varDatePaid.'</b>. <br>A minimum of 10 days is required for subsequent payments. Please renew your membership after this period.';
		...
		$argFieldsValue	= array($objMasterDB->doEscapeString($varOrderId,$objMasterDB),$objMasterDB->doEscapeString($sessMatriId,$objMasterDB),$objMasterDB->doEscapeString($varUserName,$objMasterDB),$objMasterDB->doEscapeString($varCategory,$objMasterDB),$objMasterDB->doEscapeString($varPackageCost,$objMasterDB),$objMasterDB->doEscapeString($varActualAmount,$objMasterDB),$objMasterDB->doEscapeString($varDiscountPercentage,$objMasterDB),$objMasterDB->doEscapeString($varDiscountFlatRate,$objMasterDB),$objMasterDB->doEscapeString($varCurrency,$objMasterDB),'\'3\'','NOW()',$objMasterDB->doEscapeString($varCurrency,$objMasterDB),$objMasterDB->doEscapeString($varActualAmount,$objMasterDB),$objMasterDB->doEscapeString($varPackageCost,$objMasterDB));
		...
		$varFields		= array('MatriId','User_Name','OrderId','Product_Id','Currency','Package_Cost','Amount_Paid','Discount','DiscountFlatRate','Display_Currency','Display_Amount_Paid','Display_Package_Cost','Date_Captured','Gateway');
		$argFieldsValue	= array($objMasterDB->doEscapeString($sessMatriId,$objMasterDB),$objMasterDB->doEscapeString($varUserName,$objMasterDB),$objMasterDB->doEscapeString($varOrderId,$objMasterDB),$objMasterDB->doEscapeString($varCategory,$objMasterDB),$objMasterDB->doEscapeString($varCurrency,$objMasterDB),$objMasterDB->doEscapeString($varPackageCost,$objMasterDB),$objMasterDB->doEscapeString($varActualAmount,$objMasterDB),$objMasterDB->doEscapeString($varDiscountPercentage,$objMasterDB),$objMasterDB->doEscapeString($varDiscountFlatRate,$objMasterDB),$objMasterDB->doEscapeString($varCurrency,$objMasterDB),$objMasterDB->doEscapeString($varActualAmount,$objMasterDB),$objMasterDB->doEscapeString($varPackageCost,$objMasterDB),"NOW()","4");
		$objMasterDB->insert($varTable['ONLINETOIVRCONVERSIONPAYMENTDETAILS'], $varFields, $argFieldsValue);
		...
		$varActualAmount = $varActualAmount + $varAddonActualAmount;
		}
			...
			<tr>
				...
				<tr>
					<td>
						...
						<font class="normtxt bld">Chosen Package  : </font><font class="smalltxt">  <?=$arrPrdPackages[$varCategory];?> <? if($varProHighlighter>0){echo "with Profile Highlighter";}?>   </font><font class="smalltxt"></font></td>
						...
						<td  height="25" style="padding:0px 10px 0px 10px;" class="brdr" align="center"><font class="normtxt bld">Payable Amount  : </font><font class="smalltxt"><? if ($varCountryCode=='162') { echo 'Rs'; } else { echo $arrCurrCode[$varCountryCode]; } ?> <?=$varActualAmount?> <?=$varUSDLabel?></font></td>
				...
				<tr>
					...
					<tr>
						<td height="47" width="35%" style="padding-left:15px;border-bottom: 1px solid #cbcbcb;"><font class="smalltxt">Credit card number</font></td>
