...
if($varPrimary !='' && $varSelValue[0]!=''){
	$varWhereCond	= 'WHERE '.$varWhereClause.' AND '.$varPrimary.'='.$objDB->doEscapeString($varSelValue[0],$objDB).' AND Gender='.$varGenderVal.' AND Publish=1 ORDER BY Date_Created DESC LIMIT 20';
}else{
...
for($i=0;$i<$varResultCnt;$i++)
{
	...
	echo( "<item>" );
	echo( "<title>".$varTitleValue."</title>");
	echo( "<link>".$varProfileLink."</link>" );
	echo( "<description>".preg_replace("/&/", '&amp;',$ProfileDesc)."</description>" ); 
	echo( "<pubDate>".$varResults[$i]['Date_Created']."</pubDate>" );
	echo( "<guid>".$varProfileLink."</guid>" );
	echo( "</item>" );
}
echo "</channel>";
}
