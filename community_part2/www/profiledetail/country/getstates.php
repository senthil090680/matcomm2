...
if(isset($_REQUEST["countryid"]) && $_REQUEST["countryid"]!="") {
	...
	$success_msg = "<div class='smalltxt fleft tlright pfdivlt' id='residspan'>State<span class='clr3'>*</span></div><div class='fleft pfdivrt tlleft'>";
	if($countryid == 98) {
		$success_msg.="<select  name='residingState' id='residingState' class='srchselect' onChange=modrequestnew('$countryid',this.value); onblur='ChkEmpty(document.frmProfile.residingState, \"select\", \"locresidingstate\",\"Please select the resident state of the prospect\");'>";
		...
		$success_msg.=getValuesFromArray($arrResidingPakistaniList, '- Select - ', '0', $varMemberInfo['Residing_State']);
		$success_msg.="</select>";
	}
	...
	else if($countryid == 195) {
		$success_msg.="<select  name='residingState' id='residingState' class='srchselect' onblur='ChkEmpty(document.frmProfile.residingState, \"select\", \"locresidingstate\",\"Please select the district of the prospect\");'>";

		$success_msg.=getValuesFromArray($arrResidingSrilankanList, '- Select - ', '0', $varMemberInfo['Residing_State']);
	...
	if($countryid == 98 || $countryid == 162 ) {
		...
		$success_msg.="<div class='smalltxt fleft tlright pfdivlt' id='residcitydist'>City/District<span class='clr3'>*</span></div>";
	} else {
	
		$success_msg.="<div class='smalltxt fleft tlright pfdivlt' id='residcitydist'>City<span class='clr3'>*</span></div>";
	}
