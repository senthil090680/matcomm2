...
$varFileContent		= file($varRootPath.'/data-conversion/getNewlyAddedPhotos/product-photos-list.txt');

//Delete photo based txt files
$varUncopiedFile	= $varRootPath.'/data-conversion/getNewlyAddedPhotos/Uncopied_photos.txt';
$varCopiedFile		= $varRootPath.'/data-conversion/getNewlyAddedPhotos/Copied_photos.txt';
...
for($i=0; $i<$varNoOfRecs; $i++)
{
	...
	$varSourcePath		= $varRootPath.'/data-conversion/getNewlyAddedPhotos/NewBMphotos/'.$varLangArray[strtoupper($varOldMatriId{0})].'/www/photosorg/'.$varOldMatriId{1}.'/'.$varOldMatriId{2}.'/';
	$varSourcePath2		= $varRootPath.'/data-conversion/getNewlyAddedPhotos/NewBMphotos/'.$varLangArray[$varOldMatriId{0}].'/www/photosorg/';
	$varNLSourcePath	= $varRootPath.'/data-conversion/getNewlyAddedPhotos/NewBMphotos/'.$varLangArray[strtoupper($varOldMatriId{0})].'/www/photos/'.$varOldMatriId{1}.'/'.$varOldMatriId{2}.'/';
	...
	if($varResult==1)
	{
		$varCMTableName	= $varDbaInfo['DATABASE'].'.comm_memberphotoinfo_missed';
		...
		if(count($varCopiedPhotos)>1000 || $i==($varNoOfRecs-1)){
			$varCopiedDatas	= @join("\n",$varCopiedPhotos);
	...
	else
	{
		...
		if(count($varUnCopiedPhotos)>1000 || $i==($varNoOfRecs-1)){
