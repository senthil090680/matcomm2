...
function getResultsfromSphinx($argSphinxIdxName, $argGroupbyField='',$argCommunityId){
	global $sessMatriId,$sessGender,$sessGothram,$varRootPath;
	global $objSrchBasicView,$objMasterDB,$varTempTable;//for fetching from table
	...
	if($sessMatriId != ""){
		if($varStartRec==0 && $varPostPg==0 && ($varViewedOpt==1 || $varContactOpt==1  || $varShortlistOpt==1)){	
			//Already Viewed
			$arrViewedIds = array();
			if($varViewedOpt==1){
			...
			if(count($arrTotalIds) > 0){				
				...
				if(count($arrTotalIds) < 20000){
					...
					$arrDBFieldVals = array($objMasterDB->doEscapeString($sessMatriId,$objMasterDB), "'".trim($varTotalIds, ',')."'");
					$objMasterDB->insertOnDuplicate($varTempTable,$arrDBFields,$arrDBFieldVals,'MatriId');
					$objMasterDB->dbClose();
				}
			}
		}else if($varViewedOpt==1 || $varContactOpt==1  || $varShortlistOpt==1){
			//Get Sphinx memcache value
	...
	if($sessMatriId!='') {
		...
		$s->SetSortMode(SPH_SORT_EXTENDED, "Photo_Set_Status DESC, Last_Login DESC");
	}
	...
	if(is_numeric($varActive) && $varActive>=0 && $varActive<=2){
		if($varActive==0){ 
			$s->SetFilter("OnlineStatus",array(1)); 
	...
	if($varManglik!=''  && $arrFaceting['Dosham']==1 && !in_array(0, $arrManglik)){
		...
		$s->SetFilter("Chevvai_Dosham",$arrManglik);
	}

	// SET Citizenship
	if($varCitizenship!='' && !in_array(0, $arrCitizenship)){
		$s->SetFilter("Citizenship",$arrCitizenship);
	}
	...
	if($varResidentStatus!='' && !in_array(0, $arrResidentStatus)){ 
		...
		$s->SetFilter("Resident_Status2",$arrResidentStatus);
	}
	...
	if($varOccupation!='' && !in_array(0, $arrOccupation)){
		$s->SetFilter("Occupation",$arrOccupation);
	}
