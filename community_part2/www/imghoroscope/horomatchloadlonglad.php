...
function selectArrayHashDist($countryname,$statename,$cityname,$memberid,$objSlaveDB) 
{   
	global $varTable;
    
	$varFields					= array('District','Longitude','Latitude','Timezone');
	$varCondition	            = " where District= ".$objSlaveDB->doEscapeString($cityname,$objSlaveDB);
	$districtResult				= $objSlaveDB->select($varTable['HORODISTRICT'], $varFields, $varCondition,0);
	$datastro                   = mysql_fetch_array($districtResult);
    
	$placename = $datastro["District"];
...
echo selectArrayHashDist($countryname,$statename,$cityname,$memberid,$objSlaveDB);
