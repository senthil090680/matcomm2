...
if($adminUserName == "")
	header("Location: ../indextest.php?act=login");
...
function showCommLang()
{	
	global $paLanguageKeys;
	$countCL = count($_REQUEST[commlang]);
	if($countCL <= 0)
		$disableCommLang = " disabled='true'";
	$showCommLang .= "<select id='commlang' name='commlang[]' size='3' style='width:200px;' multiple class='select' style='width:80px;' $disableCommLang>";
...
function fromdate_todate_payment($formname)
{
	if($formname == "failureoption")
	{
		...
		$fromdate_todate_payment = '<td align="center" class="smalltxt">From Date:</td><td><input type="text" name="fromdate1"  value="'.$fromDateValue.'" readonly style="width:120px;" class=\'inputtext\' onclick="displayDatePicker(\'fromdate1\', \'\', \'ymd\', \'-\');"  HIDEFOCUS></td><td nowrap="nowrap" class="smalltxt">To Date:</td><td>';
		$fromdate_todate_payment .= '<input type="text" name="todate1"  value="'.$toDateValue.'" readonly style="width:120px;" class=\'inputtext\' onclick="displayDatePicker(\'todate1\', \'\', \'ymd\', \'-\');"  HIDEFOCUS>';
	...
	if($formname == "form2")
	{
		...
		if($toDateValue == "")
			$toDateValue = date("Y-m-d",time());
		$fromdate_todate_payment = '<td align="center" class="smalltxt">From Date:</td><td><input type="text" name="fromdate"  value="'.$fromDateValue.'" readonly style="width:120px;" class=\'inputtext\' onclick="displayDatePicker(\'fromdate\', \'\', \'ymd\', \'-\');"  HIDEFOCUS></td><td class="smalltxt">To Date:</td><td>';
		$fromdate_todate_payment .= '<input type="text" name="todate"  value="'.$toDateValue.'" readonly style="width:120px;" class=\'inputtext\' onclick="displayDatePicker(\'todate\', \'\', \'ymd\', \'-\');"  HIDEFOCUS>';
		$fromdate_todate_payment .= '</td>';
	}
	if($formname == "form3")
	{
...
//	$topFiftyDomains = explode(",",$_REQUEST['topFiftyDomains']);
	...
	{
		$commLangCond = " and (";
		foreach($_REQUEST[commlang] as $clKey => $clValue)
		{
...
$time10diff = mktime(date("H"),date("i")-10,date("s"),date(m),date(d),date(Y));
$date10diff1=date("Y-m-d H:i:s",$time10diff);
//// After Suresh recommendations
	$argCondition = " where (SupportUserName='' or SupportUserName='".$adminUserName."')  and Followupstatus=0 and LockTime < '".$date10diff1."' and ((EntryType=1 and PaymentDate='0000-00-00 00:00:00') or (EntryType=0)) ".$qurydate." ".$applang." ".$commLangCond." ".$commReg;
	...
	if($category == "2")
	{
		if($fdate != "" && $tdate!="")
		{
			$qurydate= " and FollowupDate>='".$fdate."' and FollowupDate<='".$tdate."'";
			}
		else { $qurydate= "";}
  ...
  $argCondition = " where SupportUserName='".$adminUserName."' $followupStatusQ ". $applang." and ((EntryType=1 and PaymentDate='0000-00-00 00:00:00') or (EntryType=0))".$qurydate." ".$commLangCond." ".$commReg;
...
//			$topFiftyDomains = implode("~",$_REQUEST['topFiftyDomains']);
			...
			if($_REQUEST['failuresubmit'] != "") {
			$countdisp = "supportcheckstatustest.php?category=".$category."&fdate=".$fdate."&tdate=".$tdate."&leso=".$leadsources;
			}
			else {
				$countdisp = "supportcheckstatustest.php?category=".$category."&fdate=".$fdate."&tdate=".$tdate."&language=".$domain."&ts=".$ts."&leso=".$leadsources."&country=".$countycode."&commlang=$commlang&regt=".$registered."&topFiftySwitch=".$_REQUEST['topFiftySwitch']."&topFiftySwitch1=".$_REQUEST['topFiftySwitch1'].$validatedRedirect;
			...
			if($objSlave -> clsErrorCode != "CNT_ERR")
			{
				if($checkNum > 0)
				{
					...
					*/
					...
					$suppUserNameCond = " where MatriId = '$matriId'";
				...
				else
				{
					...
					$argFields = array('MatriId','FreshlyAddedOn','Country','LeadSource','QueueId','CallerId','SupportUserName','CountryCode','DateUpdated','PhoneNo','MobileNo','CommunityId','AreaCode');
					$argFieldsValue = array("'".$matriId."'","'".$curdatetime."'","'".$country."'","'".$leadSourcetxt."'","'".$queueId."'","'".$callerId."'","'".$adminUserName."'","'".$countryyycode."'","'0000-00-00 00:00:00'","'".$phoneNo."'","'".$mobileNo."'","'".$communityId."'","'".$areaCode."'");
		...
		http.onreadystatechange = function(){
			...
			if(http.readyState == 4){
				...
				//alert(response);
					...
					document.viewbyidfrm.action="supportcheckstatustest.php?category="+document.getElementById('category').value+"&mtid="+document.getElementById('mtid').value;
...
?>
    ...
    <td height=30 align=center  class="adminformheader">&nbsp;<a href='indextest.php'><b>Home</a></td>
...
<table width="100%" align=center valign="top" border=0 cellpadding=4 cellspacing=1  class="adminformheader" style="border:solid 1px #d1d1d1;">
	<tr class="rowlightbrown normaltxt1">
		...
		<td align="center" width="20%"><a href="supportcheckstatustest.php?category=4" target="_self">Pending Profiles</a></td>
...
<table width="100%" align=center valign="top" border=0 cellpadding=4 cellspacing=1 class="normaltxt1" style="border:solid 1px #d1d1d1;">
	...
	<tr>
		<td colspan=4>
		<table border=0>
		<form name="failureoption" id="failureoption" action="indextest.php" method="post">
		<tr>
	...
	<form method="post" name="form2" action="indextest.php" onsubmit="return validate_form(this)">
	<tr>
...
?>
	<div id="followup" style="<?=$outstyle?>">
      <form method="post" name="form3" action="indextest.php" onsubmit="return validate_form(this)">
	  <table width="100%" align=center valign="top" border=0 cellpadding=4 cellspacing=1 class="normaltxt1" style="border:solid 1px #d1d1d1;">
	  <tr class="rowlightbrown normaltxt1">
