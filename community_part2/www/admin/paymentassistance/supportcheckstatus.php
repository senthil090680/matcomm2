...
function lockingid($matriid,$objPaysAssMaster)
{
	...
	$argFields = array('SupportUserName');
 	$argCondition = " where SupportUserName=".$objPaysAssMaster->doEscapeString($adminUserName,$objPaysAssMaster)." and FollowupStatus=0 and MatriId=".$objPaysAssMaster->doEscapeString($fid,$objPaysAssMaster)."";
	...
	if($row_lock['SupportUserName'] == $uname && $lockcount != 0 && $fid != "")
	{
		...
		$argConditions = " MatriId=".$objPaysAssMaster->doEscapeString($fid,$objPaysAssMaster)." and (FollowupStatus=0)";
	...
	$argValues = array("'".$nowTime."'","".$objPaysAssMaster->doEscapeString($uname,$objPaysAssMaster)."");
	$argCondition = " MatriId=".$objPaysAssMaster->doEscapeString($matriid,$objPaysAssMaster)." and (SupportUserName = '' or SupportUserName=".$objPaysAssMaster->doEscapeString($uname,$objPaysAssMaster).")";
...
$argCondition = " where MatriId=".$objSlaveMatri->doEscapeString($row['MatriId'],$objSlaveMatri)."";
...
$argCondition = "where User_Name=".$objSlaveMatri->doEscapeString($uname,$objSlaveMatri)." and PrivilegeValidTime!='0000-00-00 00:00:00'";
...
if(($_REQUEST[commlang] != "") || ($PrivilegeLanguage != "" && ucwords($PrivilegeLanguage) != 'All')){
	if($commlangArr[0]!='' and  $commlangArr[0]!='0'){
			$commLangCond = " and (";
...
if($category == "1"){
	if($fdate != "" && $tdate!="")
	{
		...
		$fdateend=$tdate." 23:59:59";

		$qurydate= "FreshlyAddedOn >=".$objPaysAssMaster->doEscapeString($fdatestart,$objPaysAssMaster)." and FreshlyAddedOn <=".$objPaysAssMaster->doEscapeString($fdateend,$objPaysAssMaster)."  ";
	}
	else { $qurydate= "";}
	$nowTimeS = date("Y-m-d H:i:s");
// $argCondition = " where ".$qurydate." and ((SupportUserName='".$uname."'  and LockTime < '".$nowTimeS."') or (SupportUserName = '' and LockTime < '".$date10diff1."')) and FollowupStatus=0  and ((EntryType=1 and PaymentDate='0000-00-00 00:00:00') or EntryType=0) ". $applang."".$commLangCond.$commReg." ".$validatedQuery;
$freshtimefrm = mktime(date("H"),date("i")-25,date("s"),date(m),date(d),date(Y));
$date25mintbef=date("Y-m-d H:i:s",$freshtimefrm);
$freshNowTimeS=date("Y-m-d H:i:s");

$argCondition = " where ".$qurydate." and ((SupportUserName=".$objPaysAssMaster->doEscapeString($uname,$objPaysAssMaster)."  and LockTime < '".$nowTimeS."') or (SupportUserName = '' and LockTime <'".$freshNowTimeS."' )) and FollowupStatus=0  and ((EntryType=1 and PaymentDate='0000-00-00 00:00:00') or EntryType=0) ". $applang."".$commLangCond.$commReg." ".$validatedQuery;
...
if($category == "2"){
	if($fdate != "" && $tdate!="")
	{
		$qurydate= " FollowupDate >=".$objPaysAssMaster->doEscapeString($fdate,$objPaysAssMaster)." and FollowupDate <=".$objPaysAssMaster->doEscapeString($tdate,$objPaysAssMaster)." ";
	}
	else {$qurydate= "";}
	...
	$argCondition = " where ".$qurydate." and SupportUserName =".$objPaysAssMaster->doEscapeString($uname,$objPaysAssMaster)." $followupStatusQuery ". $applang." ".$commLangCond.$commReg." ".$validatedQuery;
	$totalcount = $objPaysAssMaster -> numOfRecords($varTable['PAYMENTOPTIONS'],'MatriId',$argCondition);
	...
	$argCondition = " where ".$qurydate." and SupportUserName=".$objPaysAssMaster->doEscapeString($uname,$objPaysAssMaster)." $followupStatusQuery ". $applang." and ((EntryType=1 and PaymentDate='0000-00-00 00:00:00') or EntryType=0) $commLangCond $validatedQuery order by DateUpdated asc limit ".$offset.",1";
...
if($category=="3")
{
	...
	if($uname != "prabhur" and $uname != "Assistance")
		$argCondition = "where ((EntryType=1 and PaymentDate='0000-00-00 00:00:00') or EntryType=0) and MatriId=".$objPaysAssMaster->doEscapeString($matriid,$objPaysAssMaster)." and ((SupportUserName <>".$objPaysAssMaster->doEscapeString($uname,$objPaysAssMaster)." and (FollowupStatus in (2,3,6,8,0) or (DateUpdated <= '$curdate' )) ) or (SupportUserName = ".$objPaysAssMaster->doEscapeString($uname,$objPaysAssMaster)." ))";
	else
		$argCondition = "where MatriId=".$objPaysAssMaster->doEscapeString($matriid,$objPaysAssMaster)." and ((EntryType=1 and PaymentDate='0000-00-00 00:00:00') or EntryType=0)";
	$totalcount = $objPaysAssMaster -> numOfRecords($varTable['PAYMENTOPTIONS'],'MatriId',$argCondition);
...
//	$argCondition = "where PaymentDate='0000-00-00 00:00:00' and (FollowupStatus = 0) and LockTime < '".$date10diff1."' and (SupportUserName = '' or SupportUserName='".$uname."')";
	$argCondition = "where PaymentDate='0000-00-00 00:00:00' and (FollowupStatus = 0) and LockTime < date_sub(now(),interval 20 minute) and (SupportUserName = '' or SupportUserName='".$uname."')";
...
$paymentCondition = " WHERE matriId = ".$objSlaveMatri->doEscapeString($row['MatriId'],$objSlaveMatri)." ";
...
$argCondition = " where MatriId =".$objSlaveMatri->doEscapeString($matriid,$objSlaveMatri)." ";
...
<html>
<head>
<script language="javascript" src="../includes/supportcheckstatus.js?ran=<?=$chkrand?>"></script>
...
<script language="javascript" src="js/hp-viewprofile.js"></script>
...
<table align=center border='0' cellpadding='0' cellspacing='0'  width='780' style='border: 1px solid #d1d1d1;'>
    <tr><td class="adminformheader"><b>Support Interface</b></td><td colspan="2" align="right" class="adminformheader"><a href='index.php'><b>Home</b></a>&nbsp;&nbsp;
...
<tr><td colspan=2>
	...
	<input type="hidden" name="matriidpost" id='matriidpost' value="<?=$recdgen['MatriId']?>">
	...
	$eprStatusCondSelect = " where ((AppointmentDate<>'0000-00-00' and AppointmentDate >= '$currDate') or (AppointmentDate='0000-00-00' and RequestDate>='$currDate 00:00:00')) and (ContactStatus<>3 and ContactStatus<>4) and MatriId =".$objEPR->doEscapeString($matriid,$objEPR)." order by  AppointmentDate desc limit 1";
	...
	if($epCount >= 1)
	{
		...
		include_once $path."/home/office/collection/www/config/arrfile.php";//contains the status arrays
		...
		for($statusCount=0;$statusCount<=$statusMaxCount;$statusCount++)
		{
			...
			$branchContactInfo = $mergedStatusArray;
			$status = $branchContactInfo[$eprResult[0]['ContactStatus']];
			$branchName = branchName($eprResult[0]['BranchId']);
		}
	...
	?>
	<div id="eprClose" style="<?php echo $displayDiv; ?>position:absolute;padding-top: 5px; width: 220px;top:31;left:670;z-index:1000;" align="right"><a href="#" onclick="srch_overlayclose('eprdiv');srch_overlayclose('eprClose');" class="smalltxt clr1"><img src="offer/close-icon.gif" alt="" border="0" height="12" width="12"></a></div>
...
function matchingcount($matriId)
{
	...
	$argCondition = " where MatriId=".$objSlaveMatri->doEscapeString($matriId,$objSlaveMatri)." and DateUpdatedOn >= '$threeDaysBack'";
