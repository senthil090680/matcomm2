...
$varFileContent		= file($varRootPath.'/newdata-conversion/photo-lists/product-photos-list'.$JJ.'.txt');

//Delete photo based txt files
$varUncopiedFile	= $varRootPath.'/newdata-conversion/photo-lists/Uncopied_photos'.$JJ.'.txt';
$varCopiedFile		= $varRootPath.'/newdata-conversion/photo-lists/Copied_photos'.$JJ.'.txt';
...
for($i=0; $i<$varNoOfRecs; $i++)
{
	...
	$varSourcePath		= $varRootPath.'/newdata-conversion/BMphotos/'.$varLangArray[strtoupper($varOldMatriId{0})].'/www/photosorg/'.$varOldMatriId{1}.'/'.$varOldMatriId{2}.'/';
	$varSourcePath2		= $varRootPath.'/newdata-conversion/BMphotos/'.$varLangArray[$varOldMatriId{0}].'/www/photosorg/';
	$varNLSourcePath	= $varRootPath.'/newdata-conversion/BMphotos/'.$varLangArray[strtoupper($varOldMatriId{0})].'/www/photos/'.$varOldMatriId{1}.'/'.$varOldMatriId{2}.'/';
	...
	if($varResult==1)
	{
		$varCMTableName	= $varDbaInfo['TEMPDATABASE'].'.comm_memberphotoinfo';
