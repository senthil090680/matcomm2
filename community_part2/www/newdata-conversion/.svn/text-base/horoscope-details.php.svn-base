...
$varFileContent		= file($varRootPath.'/newdata-conversion/horoscope-info'.$JJ.'.txt');
...
for($i=0; $i<$varNoOfLines; $i++)
{
	...
	$varHoroPath	= $varRootPath.'/newdata-conversion/BMphotos/'.$varLangArray[$varOldMatriId{0}].'/www/photos/'.$varOldMatriId{1}.'/'.$varOldMatriId{2}.'/';
	$varHoroPath2	= $varRootPath.'/newdata-conversion/BMphotos/'.$varLangArray[$varOldMatriId{0}].'/www/horoscopegen/'. $varOldMatriId{1}.'/'.$varOldMatriId{2}.'/';
	...
	$varProTable	= $varDbaInfo['TEMPDATABASE'].'.comm_memberphotoinfo';
	...
	$varCommTable		= $varDbaInfo['TEMPDATABASE'].'.comm_memberinfo';
