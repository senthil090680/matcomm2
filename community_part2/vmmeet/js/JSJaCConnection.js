...
function JSJaCReg() {
	...
	query.appendChild(iq.getDoc().createElementNS('username')).appendChild(iq.getDoc().createTextNode(this.username));
	query.appendChild(iq.getDoc().createElementNS('password')).appendChild(iq.getDoc().createTextNode(this.pass));
...
function JSJaCSASLAnonAuthBind(req) {
	...
	var eBind = iq.getDoc().createElementNS("bind");
...
function JSJaCSASLAnonAuthSess(iq) {
	...
	var eSess = iq.getDoc().createElementNS("session");
...
//alert("Call the main");
	...
	this._req[slot].onreadystatechange = function() {
		if (typeof(oCon) == 'undefined' || !oCon || !oCon.connected())
			return;
		if (oCon._req[slot].readyState == 4 || oCon._req[slot].readyState == 0) {
			oCon.oDbg.log("async recv: "+oCon._req[slot].responseText,4);
			oCon._handleResponse(oCon._req[slot]);
			
			//alert("Response "+oCon._req[slot].responseText+"\n Que length "+oCon._pQueue.length);
			
 			if (oCon._pQueue.length)
 				oCon._process();
			else{
				//alert("call _process1 "+oCon.getPollInterval());
	...
	if (typeof(this._req[slot].onerror) != 'undefined') {
		this._req[slot].onerror = function(e) {
			if (typeof(oCon) == 'undefined' || !oCon || !oCon.connected()){
				//alert("Inside async error send ocn  "+oCon+" - "+oCon.connected());
				return;
			}
			oCon.oDbg.log('XmlHttpRequest error',1);
			if (oCon._pQueue.length)
				...
				//alert("call _process2 "+oCon.getPollInterval());
				oCon._timeout = setTimeout("oCon._process()",oCon.getPollInterval()); // schedule next tick
				}
			return false;
		};
	...
	*/
	...
	// To send empty presence in all bind requests
	var xml = '';
	var empty_req = true;
	 while (this._pQueue.length) {
		...
		var reqstr = null;
		//alert("reqstr 1 : "+reqstr);
		if ( empty_req != false )
		{
		//alert("inside if");
		reqstr = this._getRequestString("<presence/>");
		empty_req = true;
		}
		else
		{
		//alert("inside else");
		reqstr = this._getRequestString(xml);
		}
		//alert("reqstr 2 : "+reqstr);
...
function JSJaCError(code,type,condition) {
	...
	doLogin();
	xmldoc.firstChild.appendChild(xmldoc.createElementNS(condition));
