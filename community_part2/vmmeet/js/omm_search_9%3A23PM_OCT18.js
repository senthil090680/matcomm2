function searchMembers( memberRecord, rQuery, ageQuery, heightQuery, citizenQuery,countryQuery, eduQuery, maritalQuery, wPhotoQuery, subcasteQuery){
	...
	//document.write(memberRecord.toString());
		//alert(" input :"+eduQuery);
		...
		for( var i=0; i<memberRecord.length; i++)
		{
			...
			{ 
				...
				{
					if (withInCRange( memberRecord[i], countryRange, countryPosition ))
					{
						if (withInCtzRange( memberRecord[i], citizenRange, citizenPosition ))
						{
							if (withInEduRange( memberRecord[i], eduRange, eduPosition))
							{
								if (withInMRange( memberRecord[i], maritalRange, mPosition))
								{
...
function withInRange( resultRecord, range, elementPosition )
{
			...
			var memberDetail = getRecordElement( resultRecord, elementPosition );
			
			//alert(memberDetail);

			for ( var i=0; i<range.length; i++ )
			{
				if(memberDetail!="undefined" && memberDetail!="null" && memberDetail!="")
				{if ( memberDetail.toLowerCase() == range[i].toLowerCase() )
				return true; }
			}
			return false;
}
...
function withInEduRange( resultRecord, range, elementPosition )
{
			...
			//alert("eduction "+memberEducation+'record:'+resultRecord);
